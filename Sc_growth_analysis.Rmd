---
title: "Example _S. cerevisiae_ growth analysis"
...
for(i in unique(strain_names)){
  ...
  strains_list[[i]] %>% mutate(OD_mean = rowMeans(select(., 1:number_of_replicates))) %>% 
    mutate(log_OD_mean = log(OD_mean)) -> strains_list[[i]]
  ...
  geom_rect(aes(xmin = index_timepoints_exp[1], xmax = Inf, ymin = -Inf, ymax = Inf), 
            fill = "dodgerblue2", alpha = 0.03) +
  theme_bw()+
  ...
  lm_fit_strains[[i]] <- lm(log_OD_mean ~ growth_data[index_timepoints_exp,1], 
                            strains_list[[i]][index_timepoints_exp,])
